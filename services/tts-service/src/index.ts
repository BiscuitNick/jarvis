...
import { TTSProviderManager, ProviderConfig } from './providers/TTSProviderManager';
import { SynthesisRequest } from './providers/TTSProvider';
...
app.get('/', (req: Request, res: Response) => {
  ...
  res.json({
    ...
    availableProviders: stats.providers.map((p: any) => p.name),
  });
});
...
app.post('/synthesize/stream', async (req: Request, res: Response) => {
  try {
    ...
    await ttsManager.synthesizeStream(
      synthesisRequest,
      (chunk: Buffer) => {
        res.write(chunk);
      },
...
app.get('/voices', async (req: Request, res: Response) => {
  try {
    ...
    const voices = await provider.listVoices(languageCode as string);
